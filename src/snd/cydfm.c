...
Uint32 get_modulator(const CydEngine *cyd, CydFm *fm) //static Uint32 get_modulator(const CydEngine *cyd, const CydFm *fm)
{
	...
	double ksl_mult = (ksl_level_final == -1) ? 1.0 : (pow(get_freq(((fm->fm_base_note << 8) + fm->fm_finetune)) * harmonic[fm->harmonic & 15] / harmonic[fm->harmonic >> 4]  /  (get_freq((fm->base_freq_for_ksl) + 1.0) * harmonic[fm->harmonic & 15] / harmonic[fm->harmonic >> 4]), (ksl_level_final == 0 ? 0 : (ksl_level_final / 127.0))));
	...
	if ((fm->flags & CYD_FM_ENABLE_WAVE) && fm->wave_entry)
	{
		...
		if(fm->counter == 10000)
		{
			debug("%f %d", ksl_mult, fm->base_freq_for_ksl);
			fm->counter = 0;
		}
...
void cydfm_set_frequency(const CydEngine *cyd, CydFm *fm, Uint32 base_frequency)
{
	...
	fm->base_freq_for_ksl = base_frequency;
